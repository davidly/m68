...
set defines=-DTARGET_BIG_ENDIAN -DM68 -DM68K

rem generate .s files for debugging
%gcc% %defines% %includes% -mcpu=68000 -x c++ -O%_optflag% ..\m68.cxx -S -fverbose-asm -o m68.s
%gcc% %defines% %includes% -mcpu=68000 -x c++ -O%_optflag% ..\m68000.cxx -S -fverbose-asm -o m68000.s

rem build the assembly portion with _start and syscalls
%gcc% %includes% -mcpu=68000 -x c++ -O%_optflag% newlib68.c -S -o newlib68.s
