# m68
m68 runs 68000 Linux elf, Motorola hex, and CP/M 68K binaries on Windows, macOS, and Linux.
...
I also tested with gcc 13.2.0 built from [https://github.com/haarer/toolchain68k](https://github.com/haarer/toolchain68k) on Windows for AMD64 and Ubuntu for AMD64 and Arm64. It almost certainly works on other platforms as well. The Ubuntu Arm64 cross compiler works on the Raspberr PI 5 as well.
One thing that doesn't work with 13.2.0 is the test application ttypes.c, which causes gcc to go into an infinite loop. I've used an ifdef to remove the offending lines. Lots of other compilers have no problem with ttypes.c.
...
M68 can run the NTVAO 6502 + Apple 1 emulator built for 68000 targeting Linux. Same for NTVCM (Z80 + CP/M 2.2), NTVDM (8086 + DOS 3.3), ARMOS (Arm64 + Linux), x64os/x32os (AMD64/x86 + Linux), SPARCOS (Sparc v8 + Linux), and RVOS (RISC-V + Linux). All of these can be found in sister repos.
...
com folder: (More information about COM can be found here: [https://github.com/johnsonjh/com-cpm/tree/master/COM](https://github.com/johnsonjh/com-cpm/tree/master/COM))
   ...
   * The Parity Even bug means that apps generated by the Manx C Vers. 1.06D 8080 compiler (and other versions) fail to run correctly.
...
aztec68k_5.2a folder: same as aztec68k but for a later version of the compiler.
