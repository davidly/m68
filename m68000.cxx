...
static inline uint32_t address_register_delta( uint16_t reg, uint16_t size )
{
    return ( 7 == reg && 0 == size ) ? 2 : ( 1u << size );
}
...
inline int32_t m68000::decode_ea_displacement( bool & is_a, bool & is_l, uint16_t & Xn )
{
    ...
    is_a = get_bit16( extension, 15 );
    Xn = get_bits16( extension, 12, 3 );
    ...
    switch( hi4 )
    {
        ...
        case 0xe: // asl / asr / lsl / lsr / ROXL / ROXR / ROL / ROR
        {
            ...
            else
            {
                ...
                if ( opbit( 5 ) )
                    tracer.Trace( "%s%c.%c d%u, d%u\n", shift_type, cdir, get_size(), op_reg, ea_reg );
                else // immediate
                {
                    uint16_t shift = ( 0 == op_reg ) ? 8 : op_reg; // only used if is_imm
                    tracer.Trace( "%s%c.%c #%u, d%u\n", shift_type, cdir, get_size(), shift, ea_reg );
                }
            }
            break;
        }
...
template <typename T> using double_width_t =
...
static inline uint32_t do_lsl( uint32_t value, uint8_t width, uint32_t count, bool &last_out )
{
    ...
    if ( count >= width )
    {
        last_out = ( count == width ) && ( 0 != ( value & 1 ) );
        return 0;
    }
...
static inline uint32_t do_lsr( uint32_t value, uint8_t width, uint32_t count, bool &last_out )
{
    ...
    if ( count >= width )
    {
        last_out = ( count == width ) && msb_width( value, width );
        return 0;
    }
...
void m68000::save_rotate( uint32_t result ) // don't modify upper bits for 8 and 16 bit rotations
{
    if ( 0 == op_size )
    {
        dregs[ ea_reg ].b = (uint8_t) result;
        set_nz8( (uint8_t) result );
    }
    else if ( 1 == op_size )
    {
        dregs[ ea_reg ].w = (uint16_t) result;
        ...
        switch ( hi4 )                   // 16% of runtime setting up for the switch
        {
            case 0: // many math and cmp instructions. I coded the below as a switch() on the second highest nibble and that was slower
            {
                ...
                else if ( 0x20 == bits11_6 || 4 == op_mode ) // btst
                {
                    ...
                    {
                        bitnum %= 32;
                        setflag_z( 0 == ( ( 1u << bitnum ) & dregs[ ea_reg ].l ) );
                    }
                    else
                    {
                        bitnum %= 8;
                        uint8_t val = effective_value8( effective_address() );
                        setflag_z( 0 == ( ( 1u << bitnum ) & val ) );
                    }
                }
                ...
                else
                {
                    ...
                    else if ( 0x22 == ( 0x2e & bits11_6 ) ) // movem
                    {
                        ...
                        if ( size_long )
                        {
                            if ( 4 == ea_mode && !memory_to_register ) // pre decrement, register to memory
                            {
                                uint32_t initial_address = aregs[ ea_reg ];
                                for ( uint16_t i = 0; i < 16; i++ )
                                {
                                    if ( mask & bit )
                                    {
                                        ...
                                        {
                                            uint16_t areg = 7 - i;
                                            setui32( aregs[ ea_reg ], ( areg == ea_reg ) ? initial_address : aregs[ areg ] );
                                        }
                                    }
                                    bit <<= 1;
                                }
                            ...
                            if ( 4 == ea_mode && !memory_to_register ) // pre decrement, register to memory
                            {
                                ...
                                for ( uint16_t i = 0; i < 16; i++ )
                                {
                                    if ( mask & bit )
                                    {
                                        ...
                                        {
                                            uint16_t areg = 7 - i;
                                            uint32_t value = ( areg == ea_reg ) ? initial_address : aregs[ areg ];
                                            setui16( aregs[ ea_reg ], (uint16_t) value );
                                        }
                                    }
                                    bit <<= 1;
                                }
                                ...
                                for ( uint16_t i = 0; i < 16; i++ )
                                {
                                    if ( mask & bit )
                                    {
                                        if ( i >= 8 )
                                        {
                                            uint16_t areg = i - 8;
                                            if ( areg != ea_reg )
                                                aregs[ areg ] = (int16_t) getui16( address );
                                        }
                                        else
            ...
            case 8: // divu / divs / sbcd / or
            {
                ...
                if ( 0x10 == bits8_4 ) // sbcd
                {
                    ...
                    {
                        aregs[ ea_reg ] -= address_register_delta( ea_reg, 0 );
                        uint8_t source = getui8( aregs[ ea_reg ] );
                        aregs[ op_reg ] -= address_register_delta( op_reg, 0 );
                        setui8( aregs[ op_reg ], bcd_sub( getui8( aregs[ op_reg ] ), source ) );
                    }
                    else // data register mode
            ...
            case 0x9: // sub / subx / suba
            {
                ...
                else if ( 0 == opbits( 4, 2 ) && opbit( 8 ) ) // subx
                {
                    if ( opbit( 3 ) ) // true if both operands are memory with a register predecrement mode, false if both are d registers
                    {
                        if ( 0 == op_size )
                        {
                            aregs[ ea_reg ] -= address_register_delta( ea_reg, op_size );
                            uint8_t source = getui8( aregs[ ea_reg ] );
                            aregs[ op_reg ] -= address_register_delta( op_reg, op_size );
                            setui8( aregs[ op_reg ], do_sub( getui8( aregs[ op_reg ] ), source, true, true, true ) );
                        }
                        else if ( 1 == op_size )
                        {
                            aregs[ ea_reg ] -= 2;
                            uint16_t source = getui16( aregs[ ea_reg ] );
                            aregs[ op_reg ] -= 2;
                            setui16( aregs[ op_reg ], do_sub( getui16( aregs[ op_reg ] ), source, true, true, true ) );
                        }
                        else
                        {
                            aregs[ ea_reg ] -= 4;
                            uint32_t source = getui32( aregs[ ea_reg ] );
                            aregs[ op_reg ] -= 4;
                            setui32( aregs[ op_reg ], do_sub( getui32( aregs[ op_reg ] ), source, true, true, true ) );
                        }
                    }
                    else
                    {
            ...
            case 0xb: // eor, cmpm, cmp, cmpa
            {
                ...
                else if ( 1 == ea_mode ) // cmpm
                {
                    if ( 0 == op_size )
                    {
                        uint8_t source = getui8( aregs[ ea_reg ] );
                        aregs[ ea_reg ] += address_register_delta( ea_reg, op_size );
                        uint8_t destination = getui8( aregs[ op_reg ] );
                        aregs[ op_reg ] += address_register_delta( op_reg, op_size );
                        do_sub( destination, source, true, false, false );
                    }
                    else if ( 1 == op_size )
                    {
                        uint16_t source = getui16( aregs[ ea_reg ] );
                        aregs[ ea_reg ] += 2;
                        uint16_t destination = getui16( aregs[ op_reg ] );
                        aregs[ op_reg ] += 2;
                        do_sub( destination, source, true, false, false );
                    }
                    else
                    {
                        uint32_t source = getui32( aregs[ ea_reg ] );
                        aregs[ ea_reg ] += 4;
                        uint32_t destination = getui32( aregs[ op_reg ] );
                        aregs[ op_reg ] += 4;
                        do_sub( destination, source, true, false, false );
                    }
                }
            ...
            case 0xc: // mulu / muls / abcd / exg / and
            {
                ...
                if ( 4 == op_mode && 0 == bits5_4 ) // abcd
                {
                    if ( opbit( 3 ) ) // address register predecrement mode
                    {
                        ...
                        uint8_t source = getui8( aregs[ ea_reg ] );
                        aregs[ op_reg ] -= address_register_delta( op_reg, 0 );
                        setui8( aregs[ op_reg ], bcd_add( source, getui8( aregs[ op_reg ] ) ) );
                    }
                    else // data register mode
                ...
                else if ( 0 == opbits( 4, 2 ) && opbit( 8 ) ) // addx
                {
                    if ( opbit( 3 ) ) // true if both operands are memory with a register predecrement mode, false if both are d registers
                    {
                        if ( 0 == op_size )
                        {
                            ...
                            uint8_t source = getui8( aregs[ ea_reg ] );
                            aregs[ op_reg ] -= address_register_delta( op_reg, op_size );
                            setui8( aregs[ op_reg ], do_add( getui8( aregs[ op_reg ] ), source, true, true, true ) );
                        }
                        else if ( 1 == op_size )
                        {
                            aregs[ ea_reg ] -= 2;
                            uint16_t source = getui16( aregs[ ea_reg ] );
                            aregs[ op_reg ] -= 2;
                            setui16( aregs[ op_reg ], do_add( getui16( aregs[ op_reg ] ), source, true, true, true ) );
                        }
                        else
                        {
                            aregs[ ea_reg ] -= 4;
                            uint32_t source = getui32( aregs[ ea_reg ] );
                            aregs[ op_reg ] -= 4;
                            setui32( aregs[ op_reg ], do_add( getui32( aregs[ op_reg ] ), source, true, true, true ) );
                        }
                    }
            ...
            case 0xe: // ASd / LSd / ROXd / ROd
            {
                ...
                else // register rotate
                {
                    ...
                    uint32_t shift = opbit( 5 ) ? ( dregs[op_reg].l & 0x3f ) : ( 0 == op_reg ) ? 8 : op_reg;
                    uint8_t width = 8 << op_size;
                    uint32_t value;
                    if ( 0 == op_size )
                        value = dregs[ ea_reg ].b;
                    else if ( 1 == op_size )
                        value = dregs[ ea_reg ].w;
                    else
                        value = dregs[ ea_reg ].l;
                    ...
                    if ( bits4_3 <= 1 ) // ASd / LSd
                    {
                        ...
                        {
                            save_rotate( do_lsl( value, width, shift, last ) );
                            setflag_v( is_arith ? asl_sign_changed( value, width, shift ) : false );
                        }
                        else // right: ASR / LSR
                        {
                            save_rotate( is_arith ? do_asr( value, width, shift, last ) : do_lsr( value, width, shift, last ) );
                            setflag_v( false );
                        }
                    ...
                    {
                        bool x = flag_x();
                        save_rotate( opbit( 8 ) ? do_roxl( value, width, shift, x ) : do_roxr( value, width, shift, x ) );
                        setflag_v( false );
                        if ( 0 == shift )
                            setflag_c( x );
                        else
                    ...
                    {
                        bool c = false;
                        save_rotate( opbit( 8 ) ? do_rol( value, width, shift, c ) : do_ror( value, width, shift, c ) );
                        setflag_v( false );
                        setflag_c( c );
                    }
                }
